        ...
        <Card className="border-slate-200 shadow-[0_20px_60px_-42px_rgba(15,23,42,0.2)]">
          <CardHeader className="flex flex-row items-center justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">
                {todayWorkout ? `${todayWorkout.focus} - ${todayWorkout.totalYardage.toLocaleString()} yds` : "No workout scheduled yet"}
              </p>
            </div>
            <Link href="/plan" className={cn(buttonVariants({ variant: "outline", className: "rounded-full border-slate-200" }))}>
              View full plan
            </Link>
          </CardHeader>
          <CardContent className="space-y-4">
            {todayWorkout ? (
              <>
                ...
                <div className="grid gap-3 md:grid-cols-2">
                  {todayWorkout.mainSet.slice(0, 2).map((set, index) => (
                    <div key={`${set.title}-${index}`} className="rounded-lg border border-slate-200 bg-white p-4">
              ...
              <div className="rounded-2xl border border-slate-200 bg-slate-50/70 p-4">
                <p className="text-sm font-semibold text-slate-950">Build your first week</p>
                <p className="mt-2 text-sm leading-7 text-slate-600">
                  Complete onboarding so DropSplit AI can create a weekly plan around your events, schedule, and goals.
                </p>
              </div>
            )}
          </CardContent>
            ...
            {[
              ...
              return (
                <Link
                  ...
                  href={action.href}
                  className={cn(
                    buttonVariants({ variant: "outline", className: "w-full justify-between rounded-lg border-slate-200 text-slate-700" }),
                  )}
                >
        ...
        <Card className="border-slate-200 shadow-[0_20px_60px_-42px_rgba(15,23,42,0.2)]">
          ...
          <CardContent className="space-y-4 text-sm text-slate-600">
            ...
            <div>
              ...
              <p className="mt-2">{data.eventInsight.primaryEvents.length ? data.eventInsight.primaryEvents.join(" - ") : "Add times first"}</p>
            </div>
            <div>
              <p className="font-semibold text-slate-950">Secondary options</p>
              <p className="mt-2">{data.eventInsight.secondaryEvents.length ? data.eventInsight.secondaryEvents.join(" - ") : "Add times first"}</p>
            </div>
          </CardContent>
        </Card>
      </section>
        ...
        <Card className="border-slate-200 shadow-[0_20px_60px_-42px_rgba(15,23,42,0.2)]">
          ...
          <CardContent className="space-y-3">
            {data.coachNotes.length ? (
              ...
              <p className="text-sm text-slate-500">Coach notes will appear after chats, logs, and workout updates.</p>
            )}
          </CardContent>
        </Card>
      </section>
